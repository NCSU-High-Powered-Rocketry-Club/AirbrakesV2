...
class MockIMU(BaseIMU):
    ...
    def __init__(
        ...
        if log_file_path is None:
            raise RuntimeError("Could not determine an IMU replay log path.")
        ...
        if self.rocket_parameters.rocket_dry_mass_kg is not None:
            airbrakes.constants.ROCKET_DRY_MASS_KG = self.rocket_parameters.rocket_dry_mass_kg
    ...
    def log_file_path(self) -> Path:
        """Return the IMU CSV replay path."""
    ...
    def _get_rocket_parameters(self, metadata: dict) -> RocketParameters:
        ...
        return RocketParameters(
            ...
            rocket_dry_mass_kg=rocket_data.get(
                "rocket_dry_mass_kg", rocket_data.get("rocket_mass_kg")
            ),
